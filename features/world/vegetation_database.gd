...
func _ready() -> void:
	...
	if config == null:
		...
		return
	for def in config.vegetation:
		_register_vegetation_spawn_definition(def)


func _register_vegetation_spawn_definition(data: VegetationSpawnDefinition) -> void:
	if data == null:
		return
	...
	if String(resolved_id).is_empty():
		...
		if path.is_empty():
			push_warning("VegetationDatabase: VegetationSpawnDefinition has no id and no resource_path; skipped.")
			return
		resolved_id = StringName(path.get_file().get_basename())
		data.id = resolved_id
	if String(resolved_id).is_empty():
		push_warning("VegetationDatabase: could not resolve VegetationSpawnDefinition id; skipped.")
		return
	if _vegetation_by_id.has(resolved_id):
		...
		return
	_vegetation_by_id[resolved_id] = data
